...
def load_dataset(path):
    ...
    for data in datasets_bar:
        ...
        defect_img = Image.open(data['defect_img_path'])
        ...
        for number in number_range:
            ...
            if not os.path.isfile(good_img_path_):
                break
            good_img = Image.open(good_img_path_)
...
def process_batch(batch, args):
    ...
    for data_text, good_imgs, defect_img in zip(text_batch, good_imgs_batch, defect_img_batch):
        ...
        response = model.generate_content(contents=input, generation_config={"temperature": 0.4}).text
        gemini_responses.append(response)

    for i, (data, response) in enumerate(zip(text_batch, gemini_responses)):
...
def main(args):
    ...
    # process the remaining batch for 3 times
    failure_count = 0
